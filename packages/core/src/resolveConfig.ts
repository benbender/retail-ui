...
const retailUIConfig = {
  ...
  theme: {
    ...
    customForms: (theme: (arg: string) => any) => ({
      default: {
        'input, textarea': {
          '&::placeholder': {
  ...
  plugins: [
    ...
    multiThemePlugin,
    // shadowOutlinePlugin
    plugin(({ addUtilities, e, theme, variants }: any) => {
      const newUtilities = {} as any
    ...
    plugin(({ addVariant }: any) => {
      addVariant('important', ({ container }: any) => {
        container.walkRules(
          (rule: {
            selector: string | any[]
            walkDecls: (arg0: (decl: any) => void) => void
          }) => {
            rule.selector = `.\\!${rule.selector.slice(1)}`
            rule.walkDecls((decl: { important: boolean }) => {
              decl.important = true
            })
          },
        )
...
function arrayMergeFn(destinationArray: any[], sourceArray: any) {
  return destinationArray.concat(sourceArray).reduce((acc: any[], cur: any) => {
...
/**
 * Merge with Tailwind CSS configurations
 * @param {object} tailwindConfig - Tailwind config object
 * @return {object} new config object
 */
export function resolveConfig(tailwindConfig: object): object {
  let purge
